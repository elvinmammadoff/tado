                ...
                if (pageNum <= max) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $container.find('img[data-lazy-src]').tadolazy();
...
// Likes for blog
  ...
  function initIsotopeMetro3() {
    ...
    /* Copyright */
    if ($('.tado_copyright_overlay').length) {
        $(document).on('contextmenu',function(event){
            if($('.tado_copyright_overlay').hasClass('copy')){
                event.preventDefault();
            }else if(event.target.tagName != 'A'){
                event.preventDefault();
            }
            $('.tado_copyright_overlay').addClass('active');
        }).on('click', function(){
            $('.tado_copyright_overlay').removeClass('active').removeAttr('style');
        });
    }
    ...
    $(window).on('load scroll resize', function () {
        if ($("img[data-lazy-src]").length) {
            $("img[data-lazy-src]").tadolazy('', function () {
                initIsotope();
            });
        }
    });
